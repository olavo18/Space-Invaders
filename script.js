...
function criarFase(){
  ...
  if(faseAtual % 10 === 0){
    ...
    // CORREÇÃO: Vida reduzida para 3000 e velocidade definida
...
function gameLoop(now){
  ...
  // CORREÇÃO BOSS 2: Esquiva + Movimento Constante
  if(boss && boss.tipo === 2 && !tempoParado && !bossMorrendo){
      // 1. SEMPRE SE MOVE LATERALMENTE
      boss.x += boss.vel * boss.dir * (dt/16);

      // 2. ESQUIVA SE HOUVER TIROS PRÓXIMOS
      tiros.forEach(t => {
          if(!t.isFireball && Math.abs(t.x - (boss.x + boss.w/2)) < 100 && t.y > boss.y && t.y < boss.y + 450){
              const forcaEsquiva = 6;
              boss.x += (t.x < boss.x + boss.w/2) ? forcaEsquiva : -forcaEsquiva;
          }
      });

      // 3. NÃO DEIXA PRENDER NA BORDA
      if(boss.x < 15) { boss.x = 15; boss.dir = 1; }
      if(boss.x > 785 - boss.w) { boss.x = 785 - boss.w; boss.dir = -1; }
  }
  ...
  invasores.forEach(inv=>{
    ...
    if((inv.isBoss || inv.isShield || !tempoParado) && !bossMorrendo){
       ...
       } else if(!inv.isBoss) { // Movimento apenas para inimigos comuns (Chefes têm lógica própria acima)
  ...
  if(edge && !bossMorrendo){ 
    invasores.forEach(e=>{ 
      if(!e.isShield && !e.isBoss){ 
        e.dir *= -1; 
        if(e.x < 10) e.x = 11;
        if(e.x > 800 - e.w - 10) e.x = 800 - e.w - 11;
        if(!tempoParado) e.y += 12; 
      } 
    });
  }
